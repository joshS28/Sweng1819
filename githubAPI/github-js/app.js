	...
	$("#search").click(function(){
		...
		function showRepos(data, status){
			...
			$("#repoDetails").children().click(function(){
				...
				// call the 'Other repositories with this name on github' function
				$(watchSubmit(repoChoice));
				document.getElementById('otherRepoDetails').style.display = "block";
		...
		var GITHUB_SEARCH_URL = 'https://api.github.com/search/repositories';
			...
			function watchSubmit(term) {
				getDataFromApi(term, displayGitHubSearchData);
			  }
		...
		var w = ($(window).width()*.50) - margin.left - margin.right;
		var h = ($(window).height()*.70) - margin.top - margin.bottom;
